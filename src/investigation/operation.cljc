(ns investigation.operation
  ...
  advisor (CaseCoordinator-LLM) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the RoutingGovernor (:govern) and
  ...
  human reviewer (case manager). The reviewer resumes with `{:approval
  {:status :approved}}` (or `:rejected`)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [investigation.llm :as llm]
            [investigation.policy :as policy]
            [investigation.phase :as phase]
...
(defn build
  ...
  opts:
    :advisor      — an `investigation.llm/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
       {:channels
        {:request     {:default nil}
         :context     {:default nil}   ; injected role/phase
      ...
      ;; CaseCoordinator-LLM inference (the contained intelligence node) — proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason (if (:flag? verdict) :compliance-concern :low-confidence))
                        :phase ph
      ...
      ;; (case manager) resumes with :approval. Then route commit/hold.
      ...
      (g/add-node :commit
        (fn [{:keys [request context proposal record]}]
          (store/commit-record! store (:op record) (:value record))
